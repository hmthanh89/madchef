...
public class Common {
	...
	public static String getRemoteInfo() {
		...
		try {
			...
			str.append(" - " + caps.getBrowserVersion() + "</b>");
	...
	private static Alert switchToAlert() {
		new WebDriverWait(Driver.getWebDriver(), Duration.ofSeconds(Constants.LOADING_TIME)).until(ExpectedConditions.alertIsPresent());
